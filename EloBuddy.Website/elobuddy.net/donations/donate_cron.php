...
$queryObj = \IPS\Db::i()->select(["i_member"], "nexus_invoices", "i_status = 'paid' AND i_total >= 5.00 AND i_id <= {$maxID} AND i_id > {$lastID}", null, null, null, null, \IPS\Db::SELECT_DISTINCT);
...
function logStatus($text){
  ...
  file_put_contents('/home/elobuddy.net/donations/update_logs.txt', "[{$date}]: {$text}" . PHP_EOL, FILE_APPEND);
}
