...
class _Votes
{
	...
	public function findByIp( $ip, \IPS\Http\Url $baseUrl = NULL )
	{
		...
		if ( $baseUrl === NULL )
		{
			return \IPS\Db::i()->select( 'COUNT(*)', 'core_voters', array( "ip_address LIKE CONCAT( ?, '%' )", $ip ) )->first();
		}
		
		/* Init Table */
		$table = new \IPS\Helpers\Table\Db( 'core_voters', $baseUrl, array( "ip_address LIKE CONCAT( ?, '%' )", $ip ) );
				
		/* Columns we need */
		$table->include = array( 'member_id', 'poll' );
	...
	{
		return \IPS\Db::i()->select( 'ip_address AS ip, 1 AS count', 'core_voters', array( 'member_id=?', $member->member_id ) )->setKeyField( 'ip' );
	}	
}